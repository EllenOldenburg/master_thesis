...
depths = unique(uniqueabmBac$Depth)

#uaB = uniqueabmBac
#uab_norm = apply(uaB[,3:ncol(uaB)], 2, function(x) (unlist(x))/max(unlist(x)))
#uniqueabmBac = cbind(uaB[,1:2],uab_norm)
#uniqueabmBac[is.na(uniqueabmBac)] = 0 # nan to 0

c = 1
cl = makeCluster(40, outfile = "") # outfile = "" print in %dopar% möglich
clusterExport(cl, ls())
registerDoParallel(cl)

start = Sys.time()
wahrsagen2_bac = foreach (i = depths) %do% {
  # print(c)
  DepthabmProphage <- uniqueabmBac[uniqueabmBac$Depth == i,]
  subProphage <- as.data.frame(DepthabmProphage)
  uaB = subProphage                                                                                                                                               
  uab_norm = apply(uaB[,3:ncol(uaB)], 2, function(x) (unlist(x))/max(unlist(x)))                                                                                   
  subProphage = cbind(uaB[,1:2],uab_norm)                                                                                                                         
  ...
  wahrsagen_bac = foreach(j = 3:ncol(subProphage), .combine = "rbind", .packages = loaded_packages) %dopar% {
    print(paste(c,j))
...
save(wahrsagen2_bac, file = "wahrsagen_bac.RData")
