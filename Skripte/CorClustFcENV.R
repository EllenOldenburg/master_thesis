...
foreach (i = 1:12, .packages = loaded_packages)%dopar%{
  ...
  CorClust_5 = foreach (j = 1:max(bestClusterForEachDepthENVForecast[[i]])) %do% {
    cluNames = paste0(depth[i],"_",names(bestClusterForEachDepthENVForecast[[i]][bestClusterForEachDepthENVForecast[[i]] == j]))
    CorClust = Korrelation175all[Korrelation175all$Org1 %in% cluNames & Korrelation175all$Org2 %in% cluNames, 1:5] 
  }
  print(str(CorClust_5))
  save(CorClust_5, file = paste0("CorClustsFcENV/CorClust_",depth[i],".RData"))
  ...
  png(filename = paste0("LagsFcENV/lags",depth[i],".png"), width = 1920 , height = 1080 )
