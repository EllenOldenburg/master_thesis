...
# setwd("~/Downloads/ellen")
# load abundance table phage and bacteria
load("uniqueEnviromentData.Rdata")
...
depths = unique(uniqueEnvi$Depth)
c = 1

testo2 = uniqueEnvi
testo3 = apply(testo2[,3:ncol(testo2)], 2, function(x) (unlist(x))/max(unlist(x), na.rm = T))
testo3[is.nan(testo3)] = 0
uniqueEnvi = cbind(testo2[,1:2],testo3)
...
wahrsagen2_env = foreach (i = depths) %do% {
  print(c)
  DepthEnv <- uniqueEnvi[uniqueEnvi$Depth == i,]
  subEnv <- as.data.frame(DepthEnv)
  
  # subEnv = subEnv[,1:(ncol(subEnv)-3)]
  subEnv = subEnv[colSums(subEnv != 0, na.rm = T) > 7]
  # testo2 = subEnv
  # testo3 = apply(testo2[,3:ncol(testo2)], 2, function(x) (unlist(x))/max(unlist(x), na.rm = T))
  # testo3[is.nan(testo3)] = 0
  # subEnv = cbind(testo2[,1:2],testo3)
  
  
  # subEnv = subEnv[which(colSums(subEnv, na.rm = T) != 0)] # remove all cols with sum 0
  # subEnv = subEnv[colSums(!is.na(subEnv)) > 1] # remove all cols containing NA only or just 1 NA
  
  
  # remove cols with the same value
  same_values = apply(subEnv[3:ncol(subEnv)], 2, function(x) !all(x==mean(x, na.rm = T)))
  subEnv = subEnv[c(T,T,same_values)]
  
  
  wahrsagen = foreach(j = 3:ncol(subEnv), .combine = "rbind", .packages = loaded_packages) %dopar% {
    subsubEnv<-subEnv[,c(2,j)]
    subsubEnv$Date <- as.factor(subEnv$Date)
    mutate(subsubEnv, Date = as.Date(Date, format = "%y%m%d"))
    
    # make data to date and ts
    z <- as.data.frame(read.zoo(subsubEnv, format = "%y%m%d"))
    z$Date <- as.yearmon(rownames(z))
    
    full = as.yearmon(2014 + seq(10, 27)/12)
    zm = data.frame(Date=full, `read.zoo(subsubEnv, format = "%y%m%d")`=with(z, `read.zoo(subsubEnv, format = "%y%m%d")`[match(full, Date)]))
    zx = read.zoo(zm)
    # plot.ts(zx)
    
    Env<-ts(zx, start = c(2014,11), end = c(2016,4), frequency = 12)
    attr(Env,"index")=NULL
    ...
    # autoplot(train_data) +
    #   ggtitle(paste0(colnames(subsubEnv)[2]," ",i," depth") )+
    #   xlab("month") + ylab("abundance")
    # ggsave(paste0(colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    #   ggtitle(paste0("Seasonal plot: ", colnames(subsubEnv)[2]," ",i," depth"))
    # ggsave(paste0("Seasonal plot: ", colnames(subsubEnv)[2]," ",i," depth.png"))
    # 
    #     ggseasonplot(train_data, polar=TRUE) + ylab("abundance") +
    #       ggtitle(paste0("Polar plot: ", colnames(subsubEnv)[2]," ",i," depth"))
    #ggsave(paste0("Polar plot: ", colnames(subsubEnv)[2]," ",i," depth.png"))
    
    ## NAIVE (Benchmark)
    model_name = "Naïve"
    model = choose_model(model_name)
    h = 13
    org = colnames(subsubEnv)[2]
    depth = i
    
    #my_forecast_plot(model,model_name, org, depth)
    #ggsave(paste0("NAIVE (Benchmark) forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    #my_forecast_plot(model,model_name, org, depth)
    #ggsave(paste0("Seasonal Naïve forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    #png(paste0("ggAcf: ", colnames(subsubEnv)[2]," ",i," depth.png"))
    # ggAcf(train_data)
    #dev.off()
    
    #png(paste0("pacf: ", colnames(subsubEnv)[2]," ",i," depth.png"))
    # pacf(train_data)
    #dev.off()
    ...
    #   #   ggtitle(paste0("Seasonal subseries plot: ", colnames(subsubEnv)[2]," ",i," depth"))
    #   # 
    #   # #ggsave(paste0("Seasonal subseries plot: ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   # 
    #   # #png(paste0("fit: ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   # autoplot(fit)
    #   # #dev.off()
    #   # 
    ...
    #   #   xlab("month") + ylab("abundance") +
    #   #   ggtitle(paste0("Seasonally adjusted plot: ", colnames(subsubEnv)[2]," ",i," depth")) +
    #   #   scale_colour_manual(values=c("gray","blue","red"),breaks=c("Data","Seasonally Adjusted","Trend"))
    #   
    #   #ggsave(paste0("Seasonally adjusted plot: ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   
    #   
    #   ### RW + DECOMPOSITION
    ...
    #   #my_forecast_plot(model,model_name, org, depth)
    #   #ggsave(paste0("RW+Decomposition forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   
    #   
    #   # compute CV RMSE
    #   cv_residuals = tsCV(train_data,model,initial = 0,h=h)
    ...
    #   # perform decomposition
    #   #my_forecast_plot(model,model_name, org, depth)
    #   #ggsave(paste0("Exp Smoothing+Decomposition forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   
    #   
    #   #png(paste0("Exp Smoothing+Decomposition forecast 24month: ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    #   
    #   #my_forecast_plot(model,model_name, org, depth)
    #   #ggsave(paste0("ARIMA + Decomposition forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   
    #   
    #   
    #   # compute CV RMSE
    ...
    #   
    #   #my_forecast_plot(model,model_name, org, depth)+
    #   # autolayer(EnvFeb, series="Feb")+
    #   # autolayer(EnvApr, series="AprMar")
    #   #ggsave(paste0("GARCH + Decomposition forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   
    #   
    #   # compute CV RMSE
    #   cv_residuals = tsCV(train_data,model,initial = 0,h=h)
    ...
    #   
    #   #my_forecast_plot(model,model_name, org, depth)
    #   #ggsave(paste0("DLM + Decomposition forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   
    #   
    #   # compute CV RMSE
    #   cv_residuals = tsCV(train_data,model,initial = 0,h=h)
    ...
    #   
    #   #my_forecast_plot(model,model_name, org, depth)
    #   #ggsave(paste0("NNETAR + Decomposition forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    #   
    #   
    #   # compute CV RMSE
    #   cv_residuals = tsCV(train_data,model,initial = 0,h=h)
    ...
    # perform decomposition
    #my_forecast_plot(model,model_name, org, depth)
    #ggsave(paste0("Exp Smoothing forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    ####### SARIMA
    xxx = auto.arima(train_data,stepwise = FALSE,approximation = FALSE)
    ...
    #my_forecast_plot(model,model_name, org, depth)
    #ggsave(paste0("SARIMA forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    #my_forecast_plot(model,model_name, org, depth)
    #ggsave(paste0("TBATS forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    #my_forecast_plot(model,model_name, org, depth)
    #ggsave(paste0("NNETAR forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    #my_forecast_plot(model,model_name, org, depth)
    #ggsave(paste0("Prophet forecast : ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    # ggsave(paste0("RMSE_plots/", best_forecast_RMSE, " forecast - ", colnames(subsubEnv)[2]," ",i," depth.png"))
    ...
    # ggsave(paste0("mae_plots/", best_forecast_mae, " forecast - ", colnames(subsubEnv)[2]," ",i," depth.png"))
...
save(wahrsagen2_env, file = "wahrsagen_env.RData")
