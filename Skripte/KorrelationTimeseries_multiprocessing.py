...
#data_list_all = ["timeseriesAll5", "timeseriesAll25","timeseriesAll45",
...
data_list_all = ["timeseriesAll500"]
data_out = "Korrelation" + data_list_all[0][13:] + ".csv"
            ...
            for data in data_list_all:
                ...
                if __name__ == '__main__':
                    ...
                    for result in pool.imap(ccf1, combinations(namsall, 2)):
                        rs = result.split("\t")
                        if (abs(float(rs[3])) >= 0.5 or abs(float(rs[6])) >= 0.5):
                            aus1.write(result)
