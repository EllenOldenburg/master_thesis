...
foreach (i = 1:12, .packages = loaded_packages)%dopar%{
  ...
  CorClust_5 = foreach (j = 1:max(bestClusterForEachDepth_wo_ENVForecast[[i]])) %do% {
    cluNames = paste0(depth[i],"_",names(bestClusterForEachDepth_wo_ENVForecast[[i]][bestClusterForEachDepth_wo_ENVForecast[[i]] == j]))
    CorClust = Korrelation175all[Korrelation175all$Org1 %in% cluNames & Korrelation175all$Org2 %in% cluNames, 1:5] 
  }
  save(CorClust_5, file = paste0("CorClustsFc/CorClust_",depth[i],".RData"))
  
  test = foreach(k = 1:length(CorClust_5), .combine = cbind) %do% {
    lag_total = table(CorClust_5[[k]]$Lag_train)
    lag_totalee = rep(0,41)
    names(lag_totalee) = -20:20
  ...
  png(filename = paste0("LagsFc/lags",depth[i],".png"), width = 1920 , height = 1080 )
